...
(function () {
    ...
    function validateTimestamps(data) {
        ...
        for (const entry of data.scoreHistory) {
            ...
            if (entry.duration < 0 || entry.duration > 86400000 * 7) { // 7日以上は異常
                return false;
            }
        }
    ...
    function validateData(data) {
        // 必須フィールドの存在確認
        if (!data.currentScore || !data.scoreHistory || 
            data.bestScore === undefined || data.totalGames === undefined || 
            data.totalMatches === undefined) {
            return false;
        }
        
        // 数値の妥当性チェック
        if (data.currentScore.matchCount < 0 || 
            data.bestScore < 0 || 
            data.totalGames < 0 || 
            data.totalMatches < 0) {
            return false;
        ...
        for (const entry of data.scoreHistory) {
            if (entry.score < 0 || entry.score > 10000) { // 10000試合は異常
                return false;
            }
        }
            ...
            if (encodedData) {
                ...
                if (parsed && typeof parsed === 'object' && parsed.currentScore) {
                    ...
                    if (validateData(parsed)) {
                        ...
                        console.error('Data validation failed, data may be corrupted or tampered');
                        // データが改ざんされている可能性がある
                        if (confirm('保存されたデータが破損しているか、不正に変更されています。データをリセットしますか？')) {
                            return createDefaultData();
                        } else {
    ...
    function resetAllData() {
        const confirmed = confirm('すべてのスコアデータをリセットしますか？この操作は取り消せません。');
        if (confirmed) {
